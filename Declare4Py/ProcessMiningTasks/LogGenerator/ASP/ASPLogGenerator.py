...
class AspGenerator(AbstractLogGenerator):
    ...
    def __init__(self,
        ...
        super().__init__(total_traces, min_event, max_event, decl_model, None, verbose)

        """Initialize ASP Generator"""
        self.__py_logger: logging.Logger = logging.getLogger("ASP generator")

        self.decl_model: DeclareModel = decl_model
        self.encode_decl_model: bool = encode_decl_model
        self.include_boundaries: bool = include_boundaries

        """ Traces Length and Traces Distribution"""
        self.positive_traces = self.total_traces
        self.negative_traces = 0

        self.traces_distribution_counter: typing.Dict[str, typing.Union[collections.Counter, None]] = {
            "positive": None,
            "negative": None
        }

        self.custom_traces_distribution_counter: typing.Dict[str, typing.Union[collections.Counter, None]] = {
            "positive": None,
            "negative": None
        }
        ...
        self.debug_message(f"traces: {self.total_traces}, events can have a trace min({self.min_events}) max({self.max_events})")
    ...
    def set_custom_traces_distribution_counter(
            self,
            custom_positive_traces_distribution: collections.Counter = None,
            custom_negative_traces_distribution: collections.Counter = None
    ):
        self.custom_traces_distribution_counter["positive"] = custom_positive_traces_distribution
        self.custom_traces_distribution_counter["negative"] = custom_negative_traces_distribution

    def compute_traces_distribution(self):

        if self.custom_traces_distribution_counter["positive"] is not None and self.custom_traces_distribution_counter["negative"] is not None:
            self.debug_message("******* Using custom traces length *******")
            self.traces_distribution_counter["positive"] = self.custom_traces_distribution_counter["positive"]
            self.traces_distribution_counter["negative"] = self.custom_traces_distribution_counter["negative"]
        else:
            self.debug_message("Using custom traces length")
            self.traces_distribution_counter["positive"] = self.compute_distribution(total_traces=self.positive_traces)
            if self.negative_traces > 0:
                self.traces_distribution_counter["negative"] = self.compute_distribution(total_traces=self.negative_traces)

    def set_traces_length(self, total_traces: typing.Union[int, None] = None, negative_traces: typing.Union[int, None] = None):

        if total_traces is not None:
            self.set_total_traces(total_traces)

        if negative_traces is not None:
            if negative_traces < 0:
                negative_traces = 0
            self.negative_traces = negative_traces

        if self.negative_traces > self.total_traces:
            raise ValueError("Negative traces can not be greater than the total number of traces")

        self.positive_traces = self.total_traces - self.negative_traces
    ...
    def run(self, clingo_config: typing.Dict[str, str] = None, generated_asp_file_path: typing.Union[str, None] = None):
        ...
        self.compute_traces_distribution()
        ...
        if self.negative_traces > 0:
            self.__py_logger.debug("Generating negative traces")
        ...
        self.__py_logger.debug("Generating traces")
        ...
        self.__py_logger.debug(f"Traces generated. Positive: {len(result['positive'])}"
        ...
        self.__py_logger.debug(f"Trace results parsed")
        self.__pm4py_log()
    ...
    def generate_asp_from_decl_model(self,
        ...
        self.__py_logger.debug("Translate declare model to ASP")
        ...
        if save_file:
            with open(save_file, 'w+') as f:
                f.write(lp)
        self.__py_logger.debug(f"Declare model translated to ASP. Total Facts {len(self.lp_model.fact_names)}")
        ...
        self.__py_logger.debug("ASP encoding generated")
        return lp
    ...
    def __generate_traces(self, lp_model: str, traces_to_generate: collections.Counter, trace_type: str, clingo_config: typing.Dict[str, str] = None):
        ...
        self.__py_logger.debug(f"Start generating traces: {traces_to_generate}")
    ...
    def __run_clingo_one_time_per_set_of_trace(self, asp: str, traces_to_generate: collections.Counter, trace_type: str, clingo_config: typing.Dict[str, str] = None):
        ...
        for num_events, num_traces in traces_to_generate.items():
            ...
            self.__py_logger.debug(f" Total trace to generate and events: Traces:{num_traces}, Events: {num_events}, RandFrequency: {freq}")
            ...
            self.__py_logger.debug(f" Clingo Result: {str(result)}")
    ...
    def __run_clingo_trace_variation(self, asp: str, num_events: int, num_traces: int, freq: float = 0.9):
        ...
        seed = randrange(0, 2 ** 30 - 1)
        self.__py_logger.debug(f" Generating variation trace: {num_traces}, events{num_events}, seed:{seed}")
        ...
        self.__py_logger.debug(f" Clingo variation Result :{str(result)}")
        if result.unsatisfiable:
    ...
    def __handle_clingo_result(self, output: clingo.solving.Model):
        ...
        self.__py_logger.debug(f" Traces generated :{symbols}")
    ...
    def __handle_clingo_variation_result(self, output: clingo.solving.Model):
        """A callback method which is given to the clingo """
        symbols = output.symbols(shown=True)
        self.__py_logger.debug(f" Variation traces generated :{symbols}")
    ...
    def __pm4py_log(self):
        ...
        """
        self.__py_logger.debug(f"Generating Pm4py log")
        decl_model: DeclareParsedDataModel = self.process_model.parsed_model
        ...
        if tot_traces_generated != self.log_length:
            ...
            if num <= 0:
                num = 1
            self.__py_logger.warning(f'PM4PY log generated: {tot_traces_generated}/{self.log_length * num} only.')
        self.__py_logger.debug(f"Pm4py generated but not saved yet")
    ...
    def set_activation_conditions_by_template_index(self, activations_list: typing.Dict[int, typing.List[int]]):
        """
    ...
    def set_custom_trace_lengths(self, custom_lengths: typing.Dict[int, int], negative_custom_lengths: typing.Union[typing.Dict[int, int], None] = None):
        ...
        self.__py_logger.info(f"****----**** Trace lengths, min_events, max_events are updated ****----****")
        self.__py_logger.info(
