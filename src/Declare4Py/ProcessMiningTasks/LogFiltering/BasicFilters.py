...
class BasicFilters:
    ...
    def filter_case_performance(self, min_performance: float, max_performance: float) -> EventLog:
        ...
        """
        if packaging.version.parse(pm4py.__version__) > packaging.version.Version("2.3.1"):
            return pm4py.filter_case_performance(self.event_log.log, min_performance, max_performance, self.event_log.timestamp_key,
    ...
    def filter_start_activities(self, activities: Union[Set[str], List[str]], retain: bool = True,
                                activity_key: str = "concept:name") -> EventLog:
        """
        ...
        """
        if packaging.version.parse(pm4py.__version__) > packaging.version.Version("2.3.1"):
            return pm4py.filter_start_activities(self.event_log.log, activities, retain, activity_key, self.event_log.timestamp_key,
                                                 self.event_log.case_id_key)
        else:
    ...
    def filter_end_activities(self, activities: [Set[str], List[str]], retain: bool = True,
        ...
        """
        if packaging.version.parse(pm4py.__version__) > packaging.version.Version("2.3.1"):
            return pm4py.filter_end_activities(self.event_log.log, activities, retain, activity_key, self.event_log.timestamp_key,
                                               self.event_log.case_id_key)
        else:
    ...
    def filter_variants_top_k(self, k: int, activity_key: str = "concept:name") -> EventLog:
        ...
        """
        if packaging.version.parse(pm4py.__version__) > packaging.version.Version("2.3.1"):
            return pm4py.filter_variants_top_k(self.event_log.log, k, activity_key, self.event_log.timestamp_key, self.event_log.case_id_key)
        else:
            variants_top_k = pm4py.filter_variants_top_k(self.event_log.log, k)
            return variants_top_k

    def filter_variants(self, variants: [Set[str], List[str]], retain: bool = True,
                        activity_key: str = "concept:name") -> EventLog:
        """
