...
class LTLAnalyzer():
    ...
    def ltl_conf_check(self, df: pandas.DataFrame) -> bool:
        ...
        result = any(self.dfa.is_accepting(state) for state in current_states)
        return result
    ...
    def run(self) -> pandas.DataFrame:
        ...
        self.data_frame['conformance'] = group['concept:name'].transform(self.ltl_conf_check)

        #Not working
        #self.data_frame['conformance'] = self.parallel_conf(group, self.ltl_conf_check)

        result_df = self.data_frame.sort_values(by=['case:concept:name'])
        #print(result_df)
