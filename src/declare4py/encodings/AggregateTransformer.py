...
class AggregateTransformer(TransformerMixin):
    ...
    def transform(self, X, y=None):
        """
        Tranforms the event log into an aggregated numeric matrix:
        ...
        if len(self.num_cols) > 0:
            dt_numeric = X.groupby(self.case_id_col)[self.num_cols].agg(['mean', 'max', 'min', 'sum', 'std']) # changed by Jonghyeon from: .agg({'mean':np.mean, 'max':np.max, 'min':np.min, 'sum':np.sum, 'std':np.std})
