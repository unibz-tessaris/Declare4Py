...
from src.declare4py.models.decl_model import TraceState
from src.declare4py.process_mining.model_checking.checker_result import CheckerResult
from src.declare4py.log_utils.encoder_declare import self.dpu.parse_data_cond, self.dpu.parse_time_cond
...
def mp_chain_response(trace, done, a, b, rules):
    activation_rules = self.dpu.parse_data_cond(self.rules["activation"])
    correlation_rules = self.dpu.parse_data_cond(rules["correlation"])
    time_rule = self.dpu.parse_time_cond(rules["time"])
...
def mp_precedence(trace, done, a, b, rules):
    activation_rules = self.dpu.parse_data_cond(rules["activation"])
    correlation_rules = self.dpu.parse_data_cond(rules["correlation"])
    time_rule = self.dpu.parse_time_cond(rules["time"])
